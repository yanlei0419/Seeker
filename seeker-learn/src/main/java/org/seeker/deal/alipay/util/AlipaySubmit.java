...
//            System.out.println(name+">>>>>"+value);
            sbHtml.append("<input type=\"hidden\" name=\"" + name + "\" value=\"" + (value==null||"null".equals(value)?"":value) + "\"/>");
        } 

        //submit按钮控件请不要含有name属性
        //sbHtml.append("<input type=\"submit\" value=\"" + strButtonName + "\"></form>");
        //sbHtml.append("<a href=\"javascript:void(0)\" onclick=\"Sub();\" >点此继续</a>");
    ...
    public static String buildRequest(Map<String, String> sParaTemp, String strMethod, String strButtonName, String strParaFileName) {
        ...
        Map<String, String> sPara = buildRequestPara(sParaTemp);
        //请求地址
        String requestUrl=  ALIPAY_GATEWAY_NEW   + "_input_charset=" + AlipayConfig.input_charset ;

        List<String> keys = new ArrayList<String>(sPara.keySet());
        StringBuffer sbHtml = new StringBuffer();
        sbHtml.append("<form id=\"alipaysubmit\" name=\"alipaysubmit\"  enctype=\"multipart/form-data\" action=\""+requestUrl+"\" method=\"" + strMethod
                      + "\">");
        for (int i = 0; i < keys.size(); i++) {
            String name = (String) keys.get(i);
            String value = (String) sPara.get(name);
